...
while read -r line ; do
  case $line in
    SYS*)
      ...
      # disk /
      diskr="%{F${color_sec_b1}}${sep_left}%{F${color_icon} B${color_back}} %{T2}${icon_hd}%{F- T1} ${sys_arr[6]}%%"
      # disk home
      diskh="%{F${color_icon}}${sep_l_left} %{T2}${icon_home}%{F- T1} ${sys_arr[7]}%%"
      # wlan
      if [ "${sys_arr[8]}" == "down" ]; then
        wland_v="×"; wlanu_v="×";
        wlan_cback=${color_sec_b2}; wlan_cicon=${color_icon}; wlan_cfore=${color_icon};
      else
      ...
      if [ "${sys_arr[10]}" == "down" ]; then
        ...
        if [ ${ethd_v:0:-3} -gt ${net_alert} ] || [ ${ethu_v:0:-3} -gt ${net_alert} ]; then
          eth_cback=${color_net}; eth_cicon=${color_back}; eth_cfore=${color_icon};
        else
          eth_cback=${color_sec_b1}; eth_cicon=${color_icon}; eth_cfore=${color_icon};
        fi
      fi
    ...
    GMA*)
      ...
      gmail="${line#???}"
      if [ "${gmail}" != "0" ]; then
        mail_cback=${color_mail}; mail_cicon=${color_icon}; mail_cfore=${color_icon}
      else
